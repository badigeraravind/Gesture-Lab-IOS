  ...
  stages {
    ...
    stage('Start Appium, grant permissions & run tests') {
      steps {
        sh ''
        '
        if !command - v applesimutils > /dev/null
        2 > & 1;
        then
        ...
        SIM_ID = $(xcrun simctl list devices booted | grep - v unavailable | head - n1 | awk - F '[()]'
          '{print $2}')
        ...
        # activate venv, install reqs and run pytest(adjust venv path
          if different)
